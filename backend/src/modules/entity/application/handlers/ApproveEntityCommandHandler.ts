import { inject, injectable } from 'inversify';
import { provide } from 'inversify-binding-decorators';

import { ICommandHandler } from '@shared/application/handlers/ICommandHandler';
import { ApproveEntityCommand } from '@modules/entity/application/commands/ApproveEntityCommand';
import { EntityId } from '@modules/entity/domain/value-objects/EntityId';
import { IEntityRepository } from '@modules/entity/domain/repositories/IEntityRepository';
import { EventBus } from '@shared/infrastructure/queue/EventBus';
import { EntityApprovedEvent } from '@modules/entity/domain/events/EntityApprovedEvent';
import { Audit } from '@shared/infrastructure/audit/AuditDecorator';

@provide(ApproveEntityCommandHandler, true)
@injectable()
export class ApproveEntityCommandHandler
  implements ICommandHandler<ApproveEntityCommand, void>
{
  constructor(
    @inject('IEntityRepository')
    private readonly entityRepository: IEntityRepository,

    @inject('EventBus')
    private readonly eventBus: EventBus
  ) {}

  @Audit('APPROVE_ENTITY', 'ENTITY')
  async handle(
    command: ApproveEntityCommand,
    userId?: string,
    ipAddress?: string
  ): Promise<void> {
    const id = EntityId.create(command.entityId);
    const entity = await this.entityRepository.findById(id);

    if (!entity) {
      throw new Error(`Entity with ID ${command.entityId} not found.`);
    }

    entity.approve();

    await this.entityRepository.save(entity);

    await this.eventBus.publish(
      new EntityApprovedEvent(entity)
    );
  }
}
