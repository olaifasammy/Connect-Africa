...
@provide(UpdateReadingProgressCommandHandler, true)
@injectable()
export class UpdateReadingProgressCommandHandler
  implements ICommandHandler<UpdateReadingProgressCommand, void>
{
  constructor(
    @inject('IContinueReadingRepository')
    private readonly repository: IContinueReadingRepository,
  ) {}
  ...
  async handle(command: UpdateReadingProgressCommand): Promise<void> {
    try {
      ...
      throw error instanceof ContinueReadingError
        ? error
        : new ContinueReadingError(
