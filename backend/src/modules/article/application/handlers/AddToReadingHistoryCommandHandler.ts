...
@injectable()
export class AddToReadingHistoryCommandHandler
  implements ICommandHandler<AddToReadingHistoryCommand, void>
{
  constructor(
    @inject('IReadingHistoryRepository')
    private readonly repository: IReadingHistoryRepository,
    @inject('EventBus')
  ...
  async handle(command: AddToReadingHistoryCommand): Promise<void> {
    try {
      ...
      throw error instanceof AuthenticationError
        ? error
        : new AuthenticationError('Failed to add to reading history');
    }
  }
}
