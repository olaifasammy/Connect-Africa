...
export class Media extends AggregateRoot<MediaProps> {
  ...
  update(props: Partial<MediaProps>): void {
    if (props.fileName) this.props.fileName = new FileName(props.fileName.value);
    if (props.mimeType) this.props.mimeType = new MimeType(props.mimeType.value);
    if (props.title) this.props.title = props.title;
    if (props.status) this.props.status = MediaStatus.create(props.status.value);
  }
