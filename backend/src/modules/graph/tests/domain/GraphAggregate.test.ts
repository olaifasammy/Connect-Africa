import { GraphAggregate } from '@modules/graph/domain/entities/GraphAggregate';
import { GraphNode, GraphEdge } from '@modules/graph/domain/entities/GraphEntities';

describe('GraphAggregate', () => {
  it('should add a node successfully', () => {
    const aggregate = new GraphAggregate();
    const node = new GraphNode('n1', 'type1', ['label1'], {});
  ...
  it('should add an edge successfully when nodes exist', () => {
    const node1 = new GraphNode('n1', 'type1', [], {});
    const node2 = new GraphNode('n2', 'type2', [], {});
    const aggregate = new GraphAggregate([node1, node2], []);
    const edge = new GraphEdge('n1', 'n2', 'relType', {});
